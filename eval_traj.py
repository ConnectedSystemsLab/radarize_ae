...
def args():
    ...
    parser.add_argument('--input', help="Directory name <odom>_<scan>_<params>.", required=True)
...
def get_stat(metric_type, pose_relation_type, data):
    ...
    if pose_relation_type not in pose_relation_types:
        raise ValueError("Invalid pose_relation type. Expected one of: %s" % pose_relation_types)
    ...
    if metric_type=='rpe':
        ...
        all_pairs = False 
        metric = metrics.RPE(pose_relation=pose_relation, delta=delta, delta_unit=delta_unit, all_pairs=all_pairs)
...
if __name__ == '__main__':
    ...
    for x in cfg['DATASET']['TEST_SPLIT']:
        ...
        rpe_translation = np.append(rpe_translation, get_stat('rpe', 'translation', data))
        ape_rotation_angle = np.append(ape_rotation_angle, get_stat('ape', 'rotation_angle', data))
        rpe_rotation_angle = np.append(ape_rotation_angle, get_stat('rpe', 'rotation_angle', data))

    np.savez(os.path.join(cfg['OUTPUT_DIR'], args.input, 'result', 'traj_eval.npz'),
