...
class RadarConfig(OrderedDict):
        ...
        for cmd, params in self.items():
            if isinstance(params[0], list):
                ...
                for param in params:
                    cfg.append(
                        " ".join(
                            [cmd]
                            + [
                                f"{x:.2f}" if type(x) is float else f"{x:}"
                                for x in param
                            ]
                        )
                    )
                ...
                cfg.append(
                    " ".join(
                        [cmd]
                        + [f"{x:.2f}" if type(x) is float else f"{x:}" for x in params]
                    )
                )
    ...
    def get_params(self):
        ...
        n_chirps = (int(self["frameCfg"][1]) - int(self["frameCfg"][0]) + 1) * self[
            "frameCfg"
        ][2]
        ...
        chirp_time = (
            self["profileCfg"][0][2] + self["profileCfg"][0][4]
        )  # Idle time + ramp time, Units in usec
        velocity_max = (3e8 / (operating_freq * 1e9)) / (
            4 * (chirp_time * 1e-6)
        )  # Units in m
