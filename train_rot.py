...
def train(net, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, batch in enumerate(tqdm(train_loader)):
        ...
        optimizer.zero_grad()
        y_pred_1 = net(x_1)
        # loss = torch.sqrt(F.mse_loss(y_pred_1, y))
        # loss_plot += loss.item()
        # loss.backward()
        # optimizer.step()
...
if __name__ == '__main__':
    ...
    train_kwargs = {'batch_size': cfg['ROTNET']['TRAIN']['BATCH_SIZE'], 
                    'drop_last': True}
    ...
    if use_cuda:
        cuda_kwargs = {'num_workers': 0,
                       'shuffle': True,
                       'worker_init_fn': lambda id: np.random.seed(id * cfg['ROTNET']['TRAIN']['SEED']),
                       }
    ...
    train_datasets = [RotationDataset(path, 
    ...
    test_datasets = [RotationDataset(path, 
                                     subsample_factor=cfg['ROTNET']['DATA']['SUBSAMPLE_FACTOR'],
    ...
    model_kwargs = {'n_channels': cfg['ROTNET']['MODEL']['N_CHANNELS'], 
    ...
    for epoch in range(1, cfg['ROTNET']['TRAIN']['EPOCHS']+ 1):
        ...
        if test_loss < least_test_loss:
            ...
            torch.save({'model_name': cfg['ROTNET']['MODEL']['NAME'],
