...
from unet.dataloader import UNetDataset
...
def confmap2range(confmap):
    ...
    bin2range = np.linspace(0, 
                            cfg['DATASET']['RR_MAX'], 
                            cfg['DATASET']['RAMAP_RSIZE'])
...
def range2confmap(range):
    ...
    confmap = np.zeros((cfg['DATASET']['RAMAP_RSIZE'], 
                        range.shape[0]))
...
if __name__ == '__main__':
    ...
    for path in npz_paths:
        ...
        # Save output
        np.savez(os.path.join(test_res_dir, os.path.basename(path)), 
                 time=d['time'],
