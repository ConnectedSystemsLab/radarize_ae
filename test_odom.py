...
import flow.model as translation_model
import rotnet.model as rotation_model
...
def compare_trajectory(pred, gt):
    ...
    for i in range(0, pred.shape[0], 100):
        ...
        ax.scatter(pred[i, 0], pred[i, 1], 
                   marker=marker, s=(35*scale)**2, c='g')
    for i in range(0, gt.shape[0], 100):
        yaw = np.rad2deg(gt[i,2])
        marker, scale = gen_arrow_head_marker(yaw)
        ax.scatter(gt[i, 0], gt[i, 1], 
                   marker=marker, s=(35*scale)**2, c='y')
    ax.set_aspect('equal')
...
class OdomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        sample = {k: torch.from_numpy(self.dataset[k][idx])\
                if type(self.dataset[k][idx]) is np.ndarray else self.dataset[k][idx] \
                for k in self.files}
        return sample
...
class OdomEstimator:
    ...
    def __call__(self, dataset):
        ...
        print('delay', delay, 'kf_delay', kf_delay, 'pos_thresh', pos_thresh, 'yaw_thresh', yaw_thresh)
            ...
            for i, batch in enumerate(tqdm(dataloader)):
                ...
                if i < kf_delay*delay:
                    ...
                    if i % delay == 0:
                        ...
                        keyframe_poses.append([pose_gt[0], pose_gt[1], quat2yaw(pose_gt[3:])])
                    continue

                # Predict translation.
                vel_pred = self.trans_model(da_heatmap).squeeze().cpu().numpy()
                dt = curr_time - timestamps[-1]
                x_pred = xs[-1] + dt * vel_pred[0] * np.cos(thetas[-1]) \
                        - dt * vel_pred[1] * np.sin(thetas[-1])
                y_pred = ys[-1] + dt * vel_pred[0] * np.sin(thetas[-1]) \
                        + dt * vel_pred[1] * np.cos(thetas[-1])

                # Predict rotation.
                stacked_ra     = torch.cat([keyframes[-kf_delay], 
                                            torch.flip(ra_heatmap, [1])], 
                                            axis=1).to(torch.float32)
                ...
                pos_constraint  = np.linalg.norm(np.array([x_pred-keyframe_poses[-kf_delay][0], 
                                                           y_pred-keyframe_poses[-kf_delay][1]])) > pos_thresh
                yaw_constraint  = np.abs(yaw_pred-keyframe_poses[-1][2]) > yaw_thresh
                if time_constraint or yaw_constraint or pos_constraint:
                    print('Adding keyframe', i)
                    last_kf = i
                    keyframes.append(ra_heatmap)
...
if __name__ == '__main__':
    ...
    # Create output dir.
    test_res_dir = os.path.join(os.path.join(cfg['OUTPUT_DIR'], cfg['ODOM']['OUTPUT_DIR']))
    ...
    for path in npz_paths:
        ...
        dataset = OdomDataset(path,
                              subsample_factor=cfg['ODOM']['PARAMS']['SUBSAMPLE_FACTOR'])
        ...
        fname = os.path.join(test_res_dir, os.path.basename(path).replace('.npz', '.png')) 
        ...
        fname = os.path.join(test_res_dir, os.path.basename(path).replace('.npz', '.txt'))
