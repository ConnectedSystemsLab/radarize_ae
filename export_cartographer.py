...
def heatmap2range(heatmap, range_bins, range_min, range_max):
    ...
    range_value = np.linspace(range_min, 
                              range_max, 
                              range_bins)
...
def sensorDataPreprocessing(odom_path, scan_path, output_path):
    ...
    valid_idx = np.logical_and(scan_times > odom_times[0], 
                               scan_times < odom_times[-1])
    ...
    odom_p_interp = interp1d(odom_times, 
                             odom_poses[:,:3], 
                             kind='linear', 
                             axis=0, 
                             fill_value='extrapolate')(scan_times)
    ...
    with rosbag.Bag(output_path, 'w') as bag:
        ...
        for i in tqdm(range(scan_heatmaps.shape[0])):
            ...
            range_msg.angle_increment = np.deg2rad((cfg['DATASET']['RA']['RA_MAX'] - cfg['DATASET']['RA']['RA_MIN']) / angle_bins)
            ...
            range_msg.ranges          = heatmap2range(np.squeeze(scan_heatmaps[i]), 
                                                      range_bins,
...
if __name__ == '__main__':
    ...
    sensorDataPreprocessing(args.odom_path,
                            args.scan_path,
                            args.output_path)
