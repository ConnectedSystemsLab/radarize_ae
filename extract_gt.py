...
if __name__ == '__main__':
    ...
    if args.npz_path:
        ...
        npz_paths = sorted(glob.glob(os.path.join(cfg['DATASET']['TEST_PATH'], '*.npz')))
    ...
    for npz_path in npz_paths:
        ...
        with np.load(npz_path) as npz:
            data = {k: npz[k] for k in npz.files\
                    if k in ['time', 'pose_gt', 'depth_map']}

        for k, v in data.items():
            print(f"{k}: {v.shape}")

        basename = 'gt_' + os.path.basename(npz_path)
        ...
        np.savetxt(os.path.join(out_dir, basename.replace('.npz', '.txt')), 
                   trajectory, delimiter=" ")

        # Save depth map.
        np.savez(os.path.join(out_dir, basename),
                 time=data['time'],
