...
from rotnet import dataloader, model
from rotnet.dataloader import RotationDataset
...
def visualize_rotation(pred, gt):
    ...
    ax[0].set_title(f"MAE x: {np.mean(np.abs(pred - gt)):.3f} RMSE x: {np.sqrt(np.mean((pred - gt)**2)):.3f}")
    ax[0].plot(pred,      label='pred', color='r')
    ax[0].plot(gt,        label='gt', color='b')
    ax[0].set_ylim(-2.0*scale_factor,2.0*scale_factor)
    ax[0].grid()

    ax[1].set_title(f"err mean x: {np.mean((pred - gt)):.3f} stdev: {np.std((pred - gt)):.3f}")
...
if __name__ == '__main__':
    ...
    # Load Trained NN
    saved_model  = torch.load(os.path.join(cfg['OUTPUT_DIR'], cfg['ROTNET']['MODEL']['NAME'], f"{cfg['ROTNET']['MODEL']['NAME']}.pth"))
    ...
    for path in npz_paths:
        ...
        im = visualize_rotation(d['rot_pred'], 
                                d['rot_gt'])
